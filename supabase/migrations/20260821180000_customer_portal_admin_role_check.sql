-- Allow the Admin onboarding RPC to validate the target user's role.
-- public.has_role intentionally restricts checks to the current caller.
...
BEGIN
  ...
  END IF;
  IF NOT authz.has_role(_user_id, 'customer'::public.app_role) THEN
