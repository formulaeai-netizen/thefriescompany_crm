...
function readMode(): AlertDispatchRunMode {
...
function readMaxPerRun(): number | undefined {
  const raw = process.argv.find((arg) => arg.startsWith("--max="))?.split("=", 2)[1];
  if (!raw) return undefined;
  const value = Number(raw);
  if (!Number.isInteger(value) || value <= 0) throw new Error("--max must be a positive integer");
  return value;
}
...
async function main() {
  ...
  if (mode === "live" && readConfirmation() !== LIVE_CONFIRMATION) {
    throw new Error(`run-alerts-once-live requires --confirm=${LIVE_CONFIRMATION}`);
  }

  const config = loadWorkerConfig();
  if (mode === "live" && !config.allowRealSend) {
    throw new Error("run-alerts-once-live requires WHATSAPP_ALLOW_REAL_SEND=true");
  }

  const supabase = createWorkerSupabaseClient(config);
  const provider = createProvider(config);
  const maxPerRun = readMaxPerRun();
...
main().catch((error: unknown) => {
  const message = error instanceof Error ? error.message : "Unknown run-alerts-once error";
  console.error("Alert dispatch run-once failed", { error: message });
  process.exitCode = 1;
});
