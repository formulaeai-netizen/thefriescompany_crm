...
const PACK_SIZE = 2.5;
const fmt = (n: number, d = 2) => Number.isFinite(n) ? n.toLocaleString("en", { maximumFractionDigits: d }) : "—";

function flagFor(variancePct: number) {
  if (variancePct < 5) return { label: "✓ Normal", className: "bg-success/15 text-success border-success/30", ai: "Normal" };
  if (variancePct <= 15) return { label: "⚠ High Wastage", className: "bg-warning/15 text-warning border-warning/30", ai: "High Wastage" };
  return { label: "🚨 Investigate", className: "bg-destructive/15 text-destructive border-destructive/30", ai: "Investigate" };
}

function ProductionPage() {
  const qc = useQueryClient();
  const { isAdmin } = useIsAdmin();
  const { data: rows = [] } = useQuery({ queryKey: ["daily_production"], queryFn: fetchProduction });
  ...
  const variancePct = actual != null && expectedPacks > 0
    ? (variancePacks / expectedPacks) * 100
    : 0;
  const hasVariance = actual != null && variancePct > 5;
  const actualWastagePct = actual != null && raw > 0
    ? (1 - (actual * PACK_SIZE) / raw) * 100
    : w;
  ...
  const save = async () => {
    ...
    setRawInput(""); setTarget(""); setNotes(""); setWastage("60"); setDate(new Date());
    setActualPacks(""); setVarianceReason("");
  ...
  const del = async (id: string) => {
    const { data: row } = await supabase.from("daily_production").select("date").eq("id", id).maybeSingle();
        ...
        <CardContent className="grid gap-4 md:grid-cols-2">
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input type="number" inputMode="decimal" min="0" step="0.1" value={rawInput} onChange={(e) => setRawInput(e.target.value)} placeholder="e.g. 100" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label>Wastage %</Label>
                <Input type="number" inputMode="decimal" min="0" max="99" step="0.1" value={wastage} onChange={(e) => setWastage(e.target.value)} />
              </div>
              <div>
            ...
            <div>
              ...
              <Input type="number" inputMode="decimal" min="0" step="1" value={target} onChange={(e) => setTarget(e.target.value)} placeholder="e.g. 50" />
            </div>
            <div>
              <Label>Notes</Label>
            ...
            <div className="border-t border-border pt-4">
              <Label className="text-xs uppercase tracking-wider text-muted-foreground">Step 2 · After production</Label>
              <div className="mt-2">
                ...
                <Input
                  ...
                  placeholder={expectedPacks > 0 ? `Expected ${fmt(expectedPacks, 1)}` : "Leave empty if not finished"}
                />
              </div>
              {hasVariance && (
                  ...
                  <div className="flex items-start gap-2 text-warning">
                    ...
                    <div>
                      ...
                      <div className="text-foreground/80">
                        ...
                        Actual wastage estimate: <strong>{fmt(actualWastagePct, 1)}%</strong> vs entered {fmt(w, 1)}%.
                      </div>
                    </div>
                  </div>
                  <div>
          ...
          <div className="space-y-4">
            <div className="rounded-lg border border-[#F59E0B]/40 bg-[rgba(245,158,11,0.08)] p-5 text-sm">
              ...
              <div className="space-y-1.5 tabular text-foreground/90">
                <div className="flex justify-between"><span>Raw Input</span><span>{fmt(raw)} kg</span></div>
                <div className="flex justify-between"><span>After Wastage ({fmt(w, 1)}%)</span><span>{fmt(usable)} kg</span></div>
                <div className="flex justify-between"><span>Pack Size</span><span>2.5 kg</span></div>
                <div className="my-2 border-t border-[#F59E0B]/30" />
                <div className="flex justify-between text-base font-semibold text-[#F59E0B]">
                  <span>Estimated Packs</span><span>{fmt(expectedPacks, 1)} packs</span>
                </div>
                {actual != null && (
            ...
            <Button onClick={save} className="w-full bg-[#F59E0B] text-[#0A0F1E] hover:bg-[#D97706]">
              Log Production
            </Button>
          </div>
        </CardContent>
        ...
        <CardContent className="p-0">
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border">
              {rows.length === 0 && (
                <tr><td colSpan={11} className="px-4 py-10 text-center text-muted-foreground">No production entries yet.</td></tr>
              )}
              {rows.map((r: any) => {
                const tgtR = r.target_packs != null ? Number(r.target_packs) : null;
                const expectedR = Number(r.packs_produced);
                const actualR = r.actual_packs_produced != null ? Number(r.actual_packs_produced) : null;
                const varPct = actualR != null && expectedR > 0
                ...
                if (tgtR != null) {
                  ...
                  badge = diff <= 0
                    ? <Badge className="bg-success/15 text-success hover:bg-success/15">✓ Met</Badge>
                    : <Badge variant="destructive">↓ {fmt(diff, 1)} short</Badge>;
                }
                return (
                  <tr key={r.id} className="hover:bg-muted/30">
                    <td className="px-4 py-3">{format(new Date(r.date), "dd MMM yyyy")}</td>
                    <td className="px-4 py-3 text-right tabular">{fmt(Number(r.raw_input_kg))}</td>
                    <td className="px-4 py-3 text-right tabular">{fmt(Number(r.wastage_percent), 1)}%</td>
                    <td className="px-4 py-3 text-right tabular">{fmt(Number(r.usable_kg))}</td>
                    <td className="px-4 py-3 text-right tabular text-muted-foreground">{fmt(expectedR, 1)}</td>
                    <td className="px-4 py-3 text-right tabular font-medium text-[#F59E0B]">
                      {actualR != null ? fmt(actualR, 1) : "—"}
                    </td>
                    <td className="px-4 py-3">
                      {fl ? <Badge variant="outline" className={fl.className}>{fl.label}</Badge> : <span className="text-muted-foreground">—</span>}
                    </td>
                    <td className="px-4 py-3 text-right tabular">{tgtR != null ? fmt(tgtR, 0) : "—"}</td>
                    <td className="px-4 py-3">{badge}</td>
                    <td className="px-4 py-3 max-w-[200px] truncate text-muted-foreground" title={r.variance_reason || r.notes || ""}>
                      {r.variance_reason ? <span className="text-warning">⚠ {r.variance_reason}</span> : (r.notes || "—")}
                    </td>
                      ...
                      {isAdmin && <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            ...
                            <AlertDialogDescription>This will permanently remove the entry from {format(new Date(r.date), "dd MMM yyyy")}.</AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
          ...
          <div className="space-y-3 p-4 md:hidden">
            {rows.length === 0 && (
              <div className="py-10 text-center text-sm text-muted-foreground">No production entries yet.</div>
            )}
            {rows.map((r: any) => {
              const tgtR = r.target_packs != null ? Number(r.target_packs) : null;
              const expectedR = Number(r.packs_produced);
              ...
              return (
                <div key={r.id} className="space-y-3 rounded-xl border border-border bg-card p-4">
                  <div className="flex items-center justify-between gap-2">
                    <span className="text-sm font-semibold text-foreground">{format(new Date(r.date), "dd MMM yyyy")}</span>
                    {fl ? <Badge variant="outline" className={fl.className}>{fl.label}</Badge> : <span className="text-xs text-muted-foreground">—</span>}
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div className="rounded-lg bg-muted/30 p-3">
                      <div className="text-[10px] uppercase tracking-wide text-muted-foreground">Raw Input</div>
                      <div className="text-base font-bold text-foreground tabular">{fmt(Number(r.raw_input_kg))} kg</div>
                    </div>
                    <div className="rounded-lg bg-muted/30 p-3">
                      <div className="text-[10px] uppercase tracking-wide text-muted-foreground">Usable ({fmt(Number(r.wastage_percent),1)}% wast.)</div>
                      <div className="text-base font-bold text-foreground tabular">{fmt(Number(r.usable_kg))} kg</div>
                    </div>
                  </div>
                  <div className="grid grid-cols-3 gap-2 text-center">
                    <div>
                      <div className="text-[10px] text-muted-foreground">Expected</div>
                      <div className="text-sm font-bold text-foreground tabular">{fmt(expectedR, 1)}</div>
                    </div>
                    <div>
                      <div className="text-[10px] text-muted-foreground">Actual</div>
                      <div className="text-sm font-bold text-primary tabular">{actualR != null ? fmt(actualR, 1) : "—"}</div>
                    </div>
                    <div>
                      <div className="text-[10px] text-muted-foreground">Variance</div>
                      <div className={cn("text-sm font-bold tabular", variancePacks > 0 ? "text-destructive" : "text-success")}>
                        {actualR != null ? (variancePacks > 0 ? `-${fmt(variancePacks,1)}` : "✓") : "—"}
                      </div>
                    </div>
                  </div>
                  {tgtR != null && (
                    <div className="text-xs text-muted-foreground">Target: {fmt(tgtR, 0)} packs</div>
                  )}
                  {r.variance_reason && (
                    <div className="rounded-lg bg-warning/10 p-2 text-xs text-warning">⚠️ {r.variance_reason}</div>
                  )}
