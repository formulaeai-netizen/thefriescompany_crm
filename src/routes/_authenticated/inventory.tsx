...
import { Plus, ArrowDownToLine, ArrowUpFromLine, Trash2, Wheat, Package as PackageIcon, Truck, Warehouse } from "lucide-react";
...
const PACK_SIZE = 2.5;
const fmtN = (n: number, d = 1) => Number.isFinite(n) ? n.toLocaleString("en", { maximumFractionDigits: d }) : "—";

function flagFor(variancePct: number) {
  if (variancePct < 5) return { label: "✓ Normal", className: "bg-success/15 text-success border-success/30" };
  if (variancePct <= 15) return { label: "⚠ High Wastage", className: "bg-warning/15 text-warning border-warning/30" };
  return { label: "🚨 Investigate", className: "bg-destructive/15 text-destructive border-destructive/30" };
}
...
function statusFor(item: any) {
  ...
  if (cur <= 0) return { label: "Out of Stock", className: "bg-destructive/15 text-destructive border-destructive/30" };
  if (cur <= min) return { label: "Low Stock", className: "bg-warning/15 text-warning border-warning/30" };
...
function InventoryPage() {
  ...
  const { data: movements = [] } = useQuery({ queryKey: ["stock_movements"], queryFn: fetchStockMovements });
  const { data: invoices = [] } = useQuery({ queryKey: ["invoices"], queryFn: fetchInvoices });
  const { data: production = [] } = useQuery({ queryKey: ["daily_production"], queryFn: fetchProduction });
  ...
  const inHandColor =
    inHand > 20 ? "text-success border-success/40" :
  ...
  const dailyLog = Array.from(dateSet).sort().reverse().map((d) => {
    ...
    const actualVals = ps.map((r) => r.actual_packs_produced != null ? Number(r.actual_packs_produced) : null);
    ...
    const variancePct = actual != null && expected > 0
      ? Math.max(0, ((expected - actual) / expected) * 100)
      : 0;
    const fl = actual != null ? flagFor(variancePct) : null;
    const delivered = (invoices as any[])
      .filter((i) => i.delivery_date === d)
      .reduce((s, i) => s + deliveredPacksFromInvoice(i), 0);
    const reasons = ps.map((r) => r.variance_reason).filter(Boolean).join(" · ");
    const notes = ps.map((r) => r.notes).filter(Boolean).join(" · ");
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Stock levels, costs and movement history.</p>
        </div>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody className="tabular">
                    {items.map((it: any) => {
                      ...
                      return (
                        <tr key={it.id} className="border-b border-border/40 hover:bg-muted/40">
                          ...
                          <td className="px-4 py-3 text-right text-muted-foreground">{Number(it.minimum_stock)}</td>
                          ...
                          <td className="px-4 py-3">
                            <div className="flex items-center justify-end gap-1">
                              <Button
                                ...
                                onClick={() => { setMoveItem(it); setMoveType("Stock In"); }}
                              >
                              ...
                              <Button
                                ...
                                onClick={() => { setMoveItem(it); setMoveType("Stock Out"); }}
                              >
                              ...
                              {isAdmin && <AlertDialog>
                                ...
                                <AlertDialogContent>
                                  <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>
                                      This deletes the item and all of its stock movements. This cannot be undone.
                                    </AlertDialogDescription>
                                  </AlertDialogHeader>
                                  <AlertDialogFooter>
                    ...
                    {items.length === 0 && (
                      <tr><td colSpan={8} className="py-10 text-center text-sm text-muted-foreground">No inventory items yet.</td></tr>
                    )}
                  </tbody>
                </table>
              </div>
          ...
          <Card>
            <CardContent className="grid gap-3 p-4 md:grid-cols-[2fr_1fr_1fr_auto]">
              ...
              <div className="flex items-end">
                <Button variant="outline" onClick={() => { setFilterItem("all"); setFromDate(""); setToDate(""); }}>
                  Reset
                </Button>
              </div>
            </CardContent>
                ...
                <tbody className="tabular">
                  {filteredMovements.map((m: any) => (
                    <tr key={m.id} className="border-b border-border/40 hover:bg-muted/40">
                      <td className="px-4 py-3 text-muted-foreground">{fmtDate(m.movement_date)}</td>
                      <td className="px-4 py-3 font-medium">{m.inventory?.item_name ?? "—"}</td>
                      <td className="px-4 py-3">
                        <Badge variant="outline" className={m.movement_type === "Stock In" ? "border-success/30 text-success" : "border-warning/30 text-warning"}>
                          {m.movement_type}
                        </Badge>
                      </td>
                      <td className="px-4 py-3 text-right">{Number(m.quantity)} {m.inventory?.unit ?? ""}</td>
                      <td className="px-4 py-3 font-mono text-xs text-muted-foreground">{m.invoices?.invoice_no ?? "—"}</td>
                      <td className="px-4 py-3 text-muted-foreground">{m.notes ?? ""}</td>
                    </tr>
                  ))}
                  {filteredMovements.length === 0 && (
                    <tr><td colSpan={6} className="py-10 text-center text-sm text-muted-foreground">No movements.</td></tr>
                  )}
                </tbody>
              </table>
            </CardContent>
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="tabular divide-y divide-border">
                  {dailyLogDesc.length === 0 && (
                    <tr><td colSpan={9} className="px-4 py-10 text-center text-muted-foreground">No production or delivery activity yet.</td></tr>
                  )}
                  {dailyLogDesc.map((r) => (
                    <tr key={r.date} className="hover:bg-muted/30">
                      ...
                      <td className="px-4 py-3 text-right text-muted-foreground">{fmtN(r.expected, 1)}</td>
                      <td className="px-4 py-3 text-right font-medium text-[#F59E0B]">{r.actual != null ? fmtN(r.actual, 1) : "—"}</td>
                      <td className="px-4 py-3 text-right">
                        {r.actual != null ? (
                          <span className={r.expected - r.actual > 0 ? "text-destructive" : "text-success"}>
                            {r.expected - r.actual > 0 ? "↓" : "↑"} {fmtN(Math.abs(r.expected - r.actual), 1)}
                          </span>
                        ) : "—"}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        {r.fl ? <Badge variant="outline" className={r.fl.className}>{r.fl.label}</Badge> : <span className="text-muted-foreground">—</span>}
                      </td>
                      <td className="px-4 py-3 max-w-[260px] truncate text-muted-foreground" title={r.reasons || r.notes}>
                        {r.reasons ? <span className="text-warning">⚠ {r.reasons}</span> : (r.notes || "—")}
                      </td>
                    </tr>
                  ))}
                </tbody>
    ...
    <DialogContent>
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            <Label>Unit</Label>
            <Input value={unit} onChange={(e) => setUnit(e.target.value)} placeholder="kg / carton / pack" />
          </div>
          <div>
        ...
        <Button onClick={submit} className="w-full">Add item</Button>
      </div>
    </DialogContent>
  );
}
...
function MovementDialog({
  ...
  return (
    <DialogContent>
      <DialogHeader>
        <DialogTitle>{type} — {item.item_name}</DialogTitle>
      </DialogHeader>
      <div className="space-y-4">
        <p className="text-xs text-muted-foreground">
          Current stock: <strong>{Number(item.current_stock)} {item.unit}</strong>
        </p>
        <div className="grid grid-cols-2 gap-3">
          <div>
        ...
        <Button onClick={submit} className="w-full">Record {type}</Button>
      </div>
    </DialogContent>
  );
}