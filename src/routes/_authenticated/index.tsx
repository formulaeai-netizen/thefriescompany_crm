...
import { calculateOutstandingAmount, isInvoiceReminderEligible } from "@/lib/invoice-reminders";
      ...
      <div className="mb-3 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Order tickets past their expected reorder cadence
          </p>
        </div>
        <span
          ...
          {rows.slice(0, 6).map((row: any) => (
            <article key={row.client.id} className="docket-card">
              <div className="flex items-start justify-between gap-3">
                ...
                <span
                  className={`docket-pill ${row.days >= thresholdDays ? "docket-pill--overdue" : "docket-pill--ontrack"}`}
                >
                  {row.days}d
                </span>
              </div>
              <div className="mt-3 grid grid-cols-2 gap-2 border-t border-dashed border-paper-ink/20 pt-3">
                <div>
                  ...
                  <div className="docket-amount mt-0.5 text-[13px]">
                ...
                <div>
                  ...
                  <div className="docket-amount mt-0.5 text-[13px]">
                    {row.avgCycle != null ? `${row.avgCycle}d` : "—"}
                  </div>
                </div>
              </div>
...
export const Route = createFileRoute("/_authenticated/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Revenue, collections, overdue invoices and data-health insights.",
      },
    ],
  }),
  component: Index,
...
function Index() {
  ...
  const cashSummaryFn = useServerFn(getCashInHandSummary);
  const cashSummaryQ = useQuery({
    queryKey: ["cash-in-hand-summary"],
    queryFn: () => cashSummaryFn({}),
  });
  const prod = useQuery({
  ...
  const invoices = useMemo(() => inv.data ?? [], [inv.data]);
  const clients = useMemo(() => cl.data ?? [], [cl.data]);
  const expenses = useMemo(() => ex.data ?? [], [ex.data]);
  const today = new Date();
  const thisMonthKey = today.toISOString().slice(0, 7);
  const lastMonthDate = new Date(today.getFullYear(), today.getMonth() - 1, 1);
  const lastMonthKey = lastMonthDate.toISOString().slice(0, 7);
  const toDateKey = useCallback((...values: Array<string | null | undefined>) => {
    const raw = values.find((v) => typeof v === "string" && v.length >= 10);
    return raw ? String(raw).slice(0, 10) : "";
  }, []);
  const invoiceDateKey = useCallback(
    (i: any) => toDateKey(i.delivery_date, i.date, i.created_at),
    [toDateKey],
  );
  const expenseDateKey = useCallback((e: any) => toDateKey(e.date, e.created_at), [toDateKey]);
  const invoiceMonthKey = useCallback((i: any) => invoiceDateKey(i).slice(0, 7), [invoiceDateKey]);
  const expenseMonthKey = useCallback((e: any) => expenseDateKey(e).slice(0, 7), [expenseDateKey]);
  ...
  const monthOptions = useMemo(() => {
    ...
    return [thisMonthKey];
  }, [invoices, expenses, thisMonthKey, invoiceMonthKey, expenseMonthKey]);
  ...
  const invMatchesMonth = (i: any) => isAll || invoiceMonthKey(i) === monthFilter;
  const expMatchesMonth = (e: any) => isAll || expenseMonthKey(e) === monthFilter;
  ...
  const pending = filteredInvoices
    ...
    .reduce((s, i) => s + Number(i.amount), 0);
  const unknown = filteredInvoices
    .filter((i) => i.payment_status === "Unknown")
    .reduce((s, i) => s + Number(i.amount), 0);
  const filteredExpensesTotal = filteredExpenses.reduce(
    (s: number, e: any) => s + Number(e.price),
    0,
  );
  // Cash in Hand is an all-time running balance (opening balance + approved
  // client payments - expenses - paid salaries), not a per-month figure -
  // it is never computed here. Sourced from the shared get_cash_in_hand_summary()
  // RPC via cashSummaryQ below, same as the P&L page.
  const cashInHand = cashSummaryQ.data?.cash_in_hand ?? 0;
  const overdue = invoices.filter((i) => isInvoiceReminderEligible(i, today));
  const payingClients = isAll
    ...
    : (() => {
        ...
        return clients.filter((c: any) => c.client_type === "Paying Client" && ids.has(c.id))
          .length;
      })();
  ...
  const overheadLastMonth = expenses
    .filter((e: any) => expenseMonthKey(e) === prevMonthKey && e.category === "Fixed Overhead")
    .reduce((s: number, e: any) => s + Number(e.price), 0);
  const overheadDelta = overheadThisMonth - overheadLastMonth;

  // Low stock
  const inventory = invy.data ?? [];
  const lowStock = inventory.filter((i: any) => Number(i.current_stock) <= Number(i.minimum_stock));
  ...
  const stats = [
    {
      key: "revenue",
      label: isAll ? "Total Revenue" : "Revenue",
      value: totalRevenue,
      format: pkr,
      icon: TrendingUp,
      accent: true,
    ...
    {
      key: "expenses",
      label: isAll ? "Total Expenses" : "Monthly Expenses",
    ...
    {
      key: "invoices_count",
      label: isAll ? "Total Invoices" : "Invoices",
  ...
  const prevInvoices = invoices.filter((i: any) => invoiceMonthKey(i) === prevMonthKey);
  ...
  const heroDelta: Record<string, { pct: number; goodUp: boolean }> = {
    revenue: {
      pct: isAll ? sharePct(collected, totalRevenue) : pctDelta(totalRevenue, prevRevenue),
      goodUp: true,
    },
    collected: {
      pct: isAll ? sharePct(collected, totalRevenue) : pctDelta(collected, prevCollected),
      goodUp: true,
    },
    // Cash in Hand is an all-time running balance, not a per-month flow, so
    // there is no meaningful "previous period" to compare it against here -
    // the ring simply reflects whether the current balance is positive.
    cash: { pct: cashInHand >= 0 ? 100 : 0, goodUp: true },
    pending: {
      pct: isAll ? sharePct(pending, totalRevenue) : pctDelta(pending, prevPending),
      goodUp: false,
    },
    expenses: {
      pct: isAll
        ? sharePct(monthlyExpenseTotal, Math.max(totalRevenue, monthlyExpenseTotal))
        : pctDelta(monthlyExpenseTotal, prevExpTotal),
      goodUp: false,
    },
    invoices_count: {
    ...
    unknown: {
      pct: isAll
        ? sharePct(unknown, totalRevenue)
        : pctDelta(
            unknown,
            prevInvoices
              .filter((i) => i.payment_status !== "Done" && i.payment_status !== "Not Done")
              .reduce((s, i) => s + Number(i.amount), 0),
          ),
      goodUp: false,
    },
  ...
  const trendKeys: string[] = isAll
    ...
    : (() => {
        ...
        return Array.from(
          { length: daysInMonth },
          (_, i) => `${monthFilter}-${String(i + 1).padStart(2, "0")}`,
        );
      })();
  ...
  const collectedSpark = trendKeys.map((key) =>
    invoices
      .filter(
        (i: any) =>
          (isAll ? invoiceMonthKey(i) : invoiceDateKey(i)) === key && i.payment_status === "Done",
      )
      .reduce((s: number, i: any) => s + Number(i.amount), 0),
  );
  ...
  return (
    <div className="space-y-6">
      <div className="ticker-strip">
        <span>
          Last updated <span className="tabular text-foreground">{lastUpdatedLabel}</span>
        </span>
        <span className="text-muted-foreground/60">·</span>
        <span>
          <span className="tabular font-semibold text-primary">{overdue.length}</span> overdue
            ...
            <div className="space-y-1 text-sm">
              ...
              {hasUnknown && (
                <p className="text-muted-foreground">
                  {pkr(unknown)} across{" "}
                  {invoices.filter((i) => i.payment_status === "Unknown").length} invoices have
                  unknown payment status (May & June + 6 April invoices). Backfill in the Invoices
                  tab.
                </p>
              )}
              {expensesGap && (
                <p className="text-muted-foreground">
                  Expense tracking has gaps after April 1, 2026 — add today's expenses in the
                  Expenses tab.
                </p>
              )}
            </div>
      ...
      {isLoading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
            {stats
              ...
              .map((s) => {
                ...
                return (
                  <Card
                    key={`${monthFilter}-${s.key}`}
                    className={`hero-card hero-card--${s.key} relative rounded-2xl`}
                  >
                    <CardContent className="relative z-10 p-5 sm:p-6">
                      ...
                      <div className="mt-2">
                        <div
                          className={`hero-card__value ${(s as any).valueClass ?? "text-foreground"}`}
                        >
                          <AnimatedNumber
                            key={`${monthFilter}-${s.key}-${s.value}`}
                            value={s.value}
                            format={s.format}
                          />
                        </div>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Cash in Hand Breakdown</CardTitle>
            </CardHeader>
            <CardContent>
              {cashSummaryQ.isLoading ? (
                <div className="py-4 text-sm text-muted-foreground">Loading cash in hand…</div>
              ) : cashSummaryQ.isError ? (
                <div className="py-4 text-sm text-destructive">
                  Could not load Cash in Hand (
                  {(cashSummaryQ.error as Error)?.message ?? "unknown error"}).
                </div>
              ) : (
                <CashInHandRows summary={cashSummaryQ.data} />
              )}
            </CardContent>
          </Card>
            ...
            {stats
              ...
              .map((s) => {
                ...
                return (
                  <Card
                    key={`${monthFilter}-${s.key}`}
                    className={`dashboard-metric-card polish-card rounded-xl ${s.accent ? "border-primary/40" : ""}`}
                  >
                    <CardContent className="p-5">
                      ...
                      <div
                        className={`font-display tabular mt-3 text-2xl font-semibold ${(s as any).valueClass ?? (s.accent ? "text-primary" : "")}`}
                      >
                        <AnimatedNumber
                          key={`${monthFilter}-${s.key}-${s.value}`}
                          value={s.value}
      ...
      {(() => {
        ...
        const overdueIdx =
          filteredInvoices.length > 0
            ? Math.round(
                (overdue.filter((i: any) => isAll || invoiceMonthKey(i) === monthFilter).length /
        ...
        return (
          <div className="grid gap-4 md:grid-cols-3">
            ...
            <div className="insight-card">
              ...
              <Link to="/pnl" className="insight-cta">
                View Details
              </Link>
            </div>
              ...
              {leader && (
                <div className="insight-map">
                  <div className="insight-map__pin" style={{ top: 44, left: "38%" }}>
                    {leader.name.length > 14 ? leader.name.slice(0, 14) + "…" : leader.name}{" "}
                    {leader.pct}%
                  </div>
                </div>
              )}
      ...
      {(() => {
        ...
        const todayDel = todayInv.reduce(
          (s: number, i: any) => s + Number(i.weight_kg ?? 0) / PS,
          0,
        );
        const monthDel = monthInv.reduce(
          (s: number, i: any) => s + Number(i.weight_kg ?? 0) / PS,
          0,
        );
        const totalProduced = (allProd as any[]).reduce((s, r) => s + packsOf(r), 0);
        const totalDelivered = invoices.reduce(
          (s: number, i: any) => s + Number(i.weight_kg ?? 0) / PS,
          0,
        );
        ...
        const cards = [
          ...
          },
          {
            ...
            primary:
              inHand === 0
                ? "Out of Stock"
                : `${inHand.toLocaleString("en", { maximumFractionDigits: 1 })} packs`,
            sub:
              inHand === 0
                ? "No stock available"
                : `${(inHand * PS).toLocaleString("en", { maximumFractionDigits: 1 })} kg current stock`,
        ...
        return (
          <div>
            <div className="mb-2 flex items-center justify-between">
              ...
              <Link to="/production" className="text-xs text-primary hover:underline">
                Log production →
              </Link>
            </div>
          ...
          <CardContent className="space-y-2 p-4">
            ...
            <div className="space-y-1 text-sm text-foreground/90">
              {(anomalies.data ?? []).slice(0, 5).map((a: any) => {
                ...
                return (
                  <div key={a.date}>
                    <span className="font-medium">{a.date}:</span> Expected {exp.toFixed(1)} packs,
                    got {act.toFixed(1)}.
                    {a.variance_reason ? (
                      <span className="text-muted-foreground"> Reason: {a.variance_reason}</span>
                    ) : null}
                  </div>
                );
              })}
      ...
      <Card className="border-primary/30">
        ...
        <CardContent>
          <div className="flex flex-wrap items-baseline gap-3">
            <div className="tabular text-2xl md:text-3xl font-semibold text-primary">
              {pkr(overheadThisMonth)}
            </div>
            {overheadLastMonth > 0 && (
                  ...
                  >
                    {expCatData.map((d, i) => (
                      <Cell
                        key={i}
                        fill={
                          GROUP_COLORS[d.name as keyof typeof GROUP_COLORS] ??
                  ...
                  <div key={i.id} className="flex items-center justify-between py-3">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <span className="tabular font-semibold">
                        {pkr(calculateOutstandingAmount(i))}
                      </span>
                    </div>
                  </div>
                );
      ...
      {(() => {
        ...
        const sentToday =
          lastSent && lastSent.toISOString().slice(0, 10) === new Date().toISOString().slice(0, 10);
        ...
        if (enabled === false) {
          ...
          icon = "✅";
          const t = lastSent.toLocaleTimeString("en", {
            hour: "numeric",
            minute: "2-digit",
            hour12: true,
          });
        ...
        return (
          <div
            className={`flex items-center justify-between rounded-md border px-4 py-2 text-xs ${cls}`}
          >
            <span>
              {icon} {text}
            </span>
            <Link to="/settings" className="underline opacity-80 hover:opacity-100">
              Settings
            </Link>
          </div>
        );
